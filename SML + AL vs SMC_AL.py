...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 24 19:13:42 2023
...
startHour = 18    # Hour of the day to start collecting data
startMinute = 33   # Minute of the day to start collecting data
dMinutes = 236     # Amount of minutes to record
...
#SMC read in
# #SMC list
# smcfile = ('/Users/katrusia/Desktop/Internship 2022-2023/SMCs_OMNI_'+year+'_mw_seasonal.txt')
...
#     duration.append(int(columns[4]))
    
# f2.close()

# # turn the data into integers arrays
# doy = np.array(DOY, dtype = int)
# dur = np.array(duration, dtype = int)
# startHour = []
# startMinute = []

# # create arrays for start hour and minute
# for i in range(0, len(startTime)):   
#     strTime = datetime.strptime(startTime[i], "%H:%M")
#     strHour = np.array(strTime.hour, dtype = int)
#     strMinute = np.array(strTime.minute, dtype = int)
    
#     #make arrays of integers values for start hour and minute
#     startHour = np.append(startHour, strHour)
#     startMinute = np.append(startMinute, strMinute)
...
domain = np.linspace((t1/60)%24, (t2/60)%24, t2 - t1) #x-axis = time in hours for that duration
...
#SMC plotting
smc_al_plot = al[t1 : t2]


fig = plt.figure()

plt.xlabel('Hours')
plt.ylabel('AL (nT), SML(nT), AL_SMC')
plt.grid()
plt.plot(domain, al_range, 'r')
plt.plot(domain, sml_range, 'g')
plt.plot(domain, smc_al_plot, 'b')

# to add a date to the title but needs to be automated
date = str(datetime.strptime('2002-12-07 18:33', '%Y-%m-%d %H:%M'))
plt.title('AL SMC for ' + date)
